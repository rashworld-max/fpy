module Svc {
    @ The mirror side of every FpySequencer port, so one instance of this
    @ component can stand in for the rest of the topology around a real
    @ FpySequencer. The implementation class is named FpySequencerTester
    @ because FpySequencer declares a friend class of that name, which gives
    @ the harness access to the sequencer's internal state (message queue,
    @ state machine, stack).
    passive component FpySequencerTester {
        ...
        @ receives sequence start notifications (from seqStartOut)
        sync input port seqStartIn: Svc.CmdSeqIn

        @ receives sequence done notifications (from seqDoneOut)
        sync input port seqDoneIn: Fw.CmdResponse

        @ receives pings (from pingOut)
        sync input port pingIn: Svc.Ping

        @ receives data the sequence pops to serial ports (from serialOut)
