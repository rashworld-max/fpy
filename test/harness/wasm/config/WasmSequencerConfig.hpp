...
//
// Harness override of the fprime-wasm default: the guest linear-memory pool
// is raised so real compiled sequences (whose linear memory is at least one
// 64 KiB wasm page) can load. Everything else matches the default.
//
// \copyright
...
/// sequencer will accept. Two pages fit every sequence the test suite builds.
constexpr FwSizeType GUEST_MEMORY_SIZE = 128 * 1024;
